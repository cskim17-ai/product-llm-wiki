---
name: bitbucket-pr
description: Use when creating, inspecting, or merging Bitbucket Cloud pull requests from Claude Code without using the browser. Uses the bundled REST API CLI with environment variables or macOS Keychain authentication.
---

# Bitbucket PR Skill

Use this skill when the user asks Claude Code to create, inspect, or merge a Bitbucket Cloud pull request without using the browser UI.
...
2. macOS Keychain + local config
   - config file: `~/.claude/bitbucket-pr/config.json`
   - Keychain service default: `claude-bitbucket-app-password`
...
```bash
mkdir -p ~/.claude/bitbucket-pr
printf '{"auth_type":"api_token_basic","username":"your-email@example.com","keychain_service":"claude-bitbucket-app-password"}\n' > ~/.claude/bitbucket-pr/config.json
chmod 600 ~/.claude/bitbucket-pr/config.json
security add-generic-password \
  -a 'your-email@example.com' \
  -s 'claude-bitbucket-app-password' \
...
Claude Code sandboxed Bash commands commonly cannot read macOS Keychain items even when the token is configured correctly. Symptoms include:
...
If this local installation uses Keychain authentication, run `bitbucket_pr.py` create/get/merge commands with the Bash sandbox disabled (`dangerouslyDisableSandbox: true`) from the first attempt. If environment-variable authentication is configured in the shell, sandboxed execution may succeed.
...
The Keychain service name is only the local Keychain service label; legacy installs may use wording such as `codex-bitbucket-app-password`. With `auth_type=api_token_basic`, the stored secret is a Bitbucket API token used as the Basic Auth password.
